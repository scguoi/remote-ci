...
make dev
```
**功能**: 根据当前目录上下文启动对应的开发服务器  
**上下文感知**:
```bash
cd backend-go && make dev      # → Go Gin服务 (:8080)
cd backend-java && make dev    # → Spring Boot (:8080)  
cd backend-python && make dev  # → FastAPI (:8000)
cd frontend-ts && make dev     # → Vite dev server (:5173)
```
